        ...
        async with aiohttp.ClientSession(cookies=cookies, headers=headers) as session:
            ...
            async with session.get(url) as response1:
                ...
                if not js_token or not log_id:
                    ...
                    return {'error': 'Failed to extract authentication tokens', 'errno': -1}
                ...
                async with session.get(list_url, params=params, headers=session_headers) as response2:
                    ...
                    if files and files[0].get('isdir') == "1":
                        ...
                        async with session.get(list_url, params=params, headers=session_headers) as response3:
                            response_data3 = await response3.json()
                            
                            if 'list' not in response_data3:
                                return {'error': 'Failed to fetch directory contents', 'errno': -1}
        ...
        async with aiohttp.ClientSession(cookies=cookies, headers=headers) as session:
            ...
            for file in files:
                ...
                file_info = {
                    ...
                    'size': await get_formatted_size(file.get('size', 0)),
